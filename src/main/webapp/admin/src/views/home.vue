...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('menpiaodinggou','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{menpiaodinggouCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>门票订购总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="menpiaodinggouChart1" class="echarts1" v-if="isAuth('menpiaodinggou','首页统计')"></div>
		<div id="menpiaodinggouChart2" class="echarts2" v-if="isAuth('menpiaodinggou','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getmenpiaodinggouCount();
		this.menpiaodinggouChat1();
		this.menpiaodinggouChat2();
	},
	methods:{
		// 统计图动画
		...
		getmenpiaodinggouCount() {
			this.$http({
				url: `menpiaodinggou/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.menpiaodinggouCount = data.data
				}
			})
		},
// 1234 竖
        ...
        var menpiaodinggouChart1 = echarts.init(document.getElementById("menpiaodinggouChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                menpiaodinggouChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    menpiaodinggouChart1.resize();
                };
            }
        });
      })
    ...
    menpiaodinggouChat2() {
      this.$nextTick(()=>{
        ...
        var menpiaodinggouChart2 = echarts.init(document.getElementById("menpiaodinggouChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                menpiaodinggouChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    menpiaodinggouChart2.resize();
                };
            }
        });
      })
